          ...
          <>
            ...
            {activeTab === 'marketing' && (
              <div className="space-y-4">
                <div className="flex gap-1">
                  {(['campaigns', 'adsets', 'ads'] as const).map(sub => (
                    <button key={sub} onClick={() => setMarketingSubTab(sub)}
                      className={`px-3 py-1.5 rounded-lg text-sm capitalize transition ${marketingSubTab === sub ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'}`}>
                      {sub === 'ads' ? 'Creatives' : sub === 'adsets' ? 'Ad Sets' : 'Campaigns'}
                    </button>
                  ))}
                </div>
                ...
                {marketingSubTab === 'campaigns' && (
                  <div className="bg-gray-900 border border-gray-800 rounded-xl">
                    <div className="p-5 border-b border-gray-800"><h2 className="text-sm font-medium text-gray-300">Campaigns</h2></div>
                    <Table
                      columns={['Campaign', 'Spend', 'Impressions', 'Clicks', 'CTR', 'CPC', 'Leads', 'CPL']}
                      rows={(metaData?.campaigns || []).map((c: any) => [
                        c.name, `$${c.spend.toLocaleString()}`, c.impressions.toLocaleString(),
                        c.clicks.toLocaleString(), `${c.ctr}%`, `$${c.cpc}`, c.leads, c.cpl ? `$${c.cpl}` : '—',
                      ])}
                    />
                  </div>
                )}

                {marketingSubTab === 'adsets' && (
                  <div className="bg-gray-900 border border-gray-800 rounded-xl">
                    <div className="p-5 border-b border-gray-800"><h2 className="text-sm font-medium text-gray-300">Ad Sets</h2></div>
                    <Table
                      columns={['Ad Set', 'Campaign', 'Spend', 'Impressions', 'Clicks', 'CTR', 'Leads', 'CPL']}
                      rows={(metaData?.adsets || []).map((a: any) => [
                        a.name, a.campaignName, `$${a.spend.toLocaleString()}`, a.impressions.toLocaleString(),
                        a.clicks.toLocaleString(), `${a.ctr}%`, a.leads, a.cpl ? `$${a.cpl}` : '—',
                      ])}
                    />
                  </div>
                )}
